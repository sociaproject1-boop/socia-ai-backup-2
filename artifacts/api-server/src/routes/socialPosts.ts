...
router.post("/posts/:id/view", async (req, res) => {
  try {
    ...
    if (authedUserId) {
      ...
      const { data, error } = await svc.rpc("record_post_view", {
        p_post_id: postId,
        p_user_id: authedUserId,
      });
      if (error) {
        // Do NOT fall back to a raw increment: that would make the same
        // account inflate the count every time the post is reopened.
        logger.warn({ err: error, postId, userId: authedUserId }, "[posts/view] unique view RPC unavailable");
        res.json({ ok: true, counted: false, reason: "view_tracking_not_ready" });
        return;
      }
      res.json({ ok: true, counted: data === true });
      return;
    } else {
      ...
      if (!anonHasSeen(fingerprint, postId)) {
        ...
        try {
          await svc.rpc("increment_post_views", { post_id: postId });
          counted = true;
        } catch { /* ok */ }
      }
    }
