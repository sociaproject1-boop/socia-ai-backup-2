...
export const FeedCard = memo(function FeedCard({
  ...
  const handleLike = useCallback((e: React.MouseEvent) => {
    ...
    requireAuth(async () => {
      if (onLike) {
        ...
        // Self-managed optimistic update
        ...
        try {
          if (nowLiked) await supabase.rpc("like_post" as any, { post_id: post.id });
          else           await supabase.rpc("unlike_post" as any, { post_id: post.id });
        } catch {
          setLocalLiked(!nowLiked);
          setLocalLikeCount((c) => c + (nowLiked ? -1 : 1));
        }
  ...
  }, [onLike, post.id, localLiked, requireAuth]);
      ...
      if (!displayLiked) {
        requireAuth(() => {
          if (onLike) {
            ...
            supabase.rpc("like_post" as any, { post_id: post.id }).catch(() => {
              setLocalLiked(false);
              setLocalLikeCount((c) => c - 1);
            });
          }
  ...
  }, [displayLiked, post, onLike, onOpenViewer, navigate, requireAuth]);
