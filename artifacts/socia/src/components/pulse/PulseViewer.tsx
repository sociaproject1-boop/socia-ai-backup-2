 ...
 *  • Tap left half → prev pulse, tap right half → next pulse
 *  • Hold anywhere → pause
 *  • Swipe down → close
...
import { X, Eye, Trash2, Flag, Heart, ThumbsUp, Flame, ChevronLeft, ChevronRight } from "lucide-react";
...
export function PulseViewer({
  ...
  useEffect(() => {
    if (!pulse) return;
    let cancelled = false;
    recordPulseView(pulse.id).then((r) => {
      if (cancelled) return;
      setViewCount(r.view_count);
      onMarkViewed?.(pulse.id);
    }).catch(() => {});
  ...
  const advance = useCallback(() => {
    const nextPulseIdx = pulseIdx + 1;
    if (nextPulseIdx < availPulses.length) {
      setPulseIdx(nextPulseIdx);
      setProgress(0);
      elapsed.current = 0;
    } else {
  ...
  useEffect(() => {
    ...
    if (pulse.type === "video") return; // video controls itself
  ...
  }, [pulse?.id, paused, advance, duration]);
  ...
  const handlePointerDown = useCallback((e: React.PointerEvent) => {
    ...
    holdTimer.current = setTimeout(() => { setPaused(true); videoRef.current?.pause(); }, 150);
  }, []);
  ...
  const handleDelete = useCallback(async () => {
    ...
    try {
      ...
      setLocalDeleted((prev) => new Set([...prev, pulse.id]));
      advance();
    } catch { /* ignore */ }
  }, [pulse, advance]);
  ...
  return createPortal(
    <AnimatePresence>
      ...
      >
        ...
        <div className="relative z-10 pt-safe">
          ...
          <div className="flex items-center gap-3 px-3 pt-2 pb-1">
            <div className="h-8 w-8 rounded-full overflow-hidden flex-shrink-0"
              style={{ outline: "2px solid rgba(255,255,255,0.3)" }}>
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-[10px] text-white/60">{timeAgo(pulse.created_at)}</p>
            </div>
        ...
        {/* ── Bottom bar ──────────────────────────────────────────────── */}
        <div
          className="relative z-10 flex items-center gap-3 px-4 pb-safe pt-3 pb-4"
          ...
          <AnimatePresence>
            {showReacts && (
              <motion.div
                ...
                className="absolute bottom-16 right-4 flex gap-2 rounded-2xl p-2"
...
/* ── PulseViewerHost — mounted once in App, listens for open events ──────── */
...
/* ── Utility: open viewer from anywhere ─────────────────────────────────── */

export function openPulseViewer(
  groups: PulseFeedGroup[],
  groupIndex: number,
  pulseIndex = 0
) {
  window.dispatchEvent(
    new CustomEvent("socia:open-pulse", {
      detail: { groups, groupIndex, pulseIndex },
    })
  );
}
