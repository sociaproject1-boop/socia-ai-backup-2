...
export function AuthProvider({ children }: { children: ReactNode }) {
    ...
    try {
      ...
      if (dbProfile) {
        ...
        if (localTs > dbTs && cached) {
          ...
          setUser({
            id:         sbUser.id,
            name:       (cached.name     as string | undefined) || dbProfile.name       || sbUser.email?.split("@")[0] || "Socia User",
            handle:     (cached.username as string | undefined) || dbProfile.username   || sbUser.email?.split("@")[0] || "user",
        ...
        setUser({
          id:         sbUser.id,
          name:       dbProfile.name       || sbUser.email?.split("@")[0] || "Socia User",
          handle:     dbProfile.username   || sbUser.email?.split("@")[0] || "user",
  ...
  async function signUpEmail(email: string, password: string, displayName: string) {
    ...
    if (data.user && !data.session) {
      ...
      // Use ensureProfile (upsert) so the row is created even on first signup.
