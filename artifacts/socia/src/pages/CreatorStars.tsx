...
import { ArrowLeft, Lock, Star, Heart, Sparkles, Gift, Award, ChevronRight, Zap } from "lucide-react";
...
const FEATURES = [
  { icon: Star,     title: "Send Stars",           desc: "Fans send stars to support their favourite creators instantly." },
  { icon: Heart,    title: "Fan Support",           desc: "Recurring or one-time support flows from fans to creators." },
  { icon: Sparkles, title: "Creator Badges",        desc: "Earn 'Rising Star', 'Top Creator', and premium badge tiers." },
  { icon: Gift,     title: "Star Gifts",            desc: "Animated star gifts displayed live in creator streams and posts." },
  { icon: Award,    title: "Creator Leaderboard",   desc: "Top-supported creators featured weekly on the platform." },
  { icon: Zap,      title: "Star Multipliers",      desc: "Premium creators earn bonus stars during campaigns and events." },
];

const REQUIREMENTS = ["Public creator account", "Active content", "Policy compliant", "Stars feature enrollment"];

const ROLLOUT_STEPS = [
  { label: "Funding goal reached", active: true },
  { label: "7–15 days infrastructure prep", active: false },
  { label: "Creator verification system ready", active: false },
  { label: "Limited beta — enrolled creators first", active: false },
  { label: "Gradual public access", active: false },
];
  ...
  return (
    <div className="app-bg min-h-[100dvh] pb-28">
      <div className="app-header sticky top-0 z-40 flex items-center gap-3 px-4 py-3">
        ...
        <h1 className="text-lg font-bold app-text">Creator Stars</h1>
      </div>
  ...
  return (
    <div className="relative overflow-hidden rounded-[22px] p-5 text-center"
      ...
      <p className="mb-4 text-[11.5px] leading-relaxed text-white/50" style={{ maxWidth: 270, margin: "0 auto 16px" }}>
        Funding goal completion unlocks the next deployment phase. The Stars system rolls out gradually after creator verification infrastructure and secure payment rails are in place.
      </p>
...
function RequirementsCard({ items }: { items: string[] }) {
  return (
    <div className="mt-4 rounded-[18px] p-4" style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.07)" }}>
      <p className="mb-3 text-[10.5px] font-semibold uppercase tracking-wider app-text-muted">Requirements</p>
