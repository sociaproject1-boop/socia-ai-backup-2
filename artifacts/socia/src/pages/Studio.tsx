...
function BadgeChip({ badge, absolute = true }: { badge: string; absolute?: boolean }) {
  ...
  return (
    <div
      className={`${absolute ? "absolute right-2 top-2" : ""} inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[9px] font-bold tracking-wide`}
    ...
    >
      ...
      >
        ...
        <div className="relative aspect-[3/4] overflow-hidden">
          ...
          {/* Cinematic bottom gradient — readable text + premium depth */}
          <div className="absolute inset-0"
            style={{ background: "linear-gradient(to top, rgba(0,0,0,0.85) 0%, rgba(0,0,0,0.45) 35%, rgba(0,0,0,0.08) 60%, transparent 100%)" }} />
          ...
          {/* Kind + spec pill (glassmorphism) */}
          <div className="absolute left-2 top-2 inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[10px] font-medium text-white"
          ...
          {/* Upload required indicator — glass pill with icon */}
          {needsUpload && (
            <div className="absolute bottom-2 left-2 inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[9px] font-medium text-white/85"
              style={{ background: "rgba(10,10,10,0.72)", backdropFilter: "blur(10px)", border: "1px solid rgba(255,255,255,0.08)" }}>
          ...
          {/* Title overlay — sits ON the image for cinema-poster feel */}
          <div className="absolute inset-x-0 bottom-0 p-2.5">
            <h3 className="line-clamp-1 text-[13.5px] font-bold leading-tight text-white drop-shadow">
              {preset.title}
            </h3>
            <p className="mt-0.5 line-clamp-1 text-[10px] uppercase tracking-wider text-white/55">
              {preset.category}
            </p>
          </div>
        </div>
          ...
          {preset.tags && preset.tags.length > 0 && (
            <div className="mt-1.5 flex flex-wrap gap-1">
              {preset.tags.slice(0, 2).map((tag) => (
                <span key={tag}
                  className="rounded-full px-1.5 py-0.5 text-[9px] text-white/50"
        ...
        <motion.div
          ...
          className="absolute inset-x-0 bottom-0 flex items-center justify-center gap-1.5 rounded-b-2xl py-2.5 text-[11.5px] font-bold text-white"
      ...
      {/* Favorite — top-right of card */}
      <button
        onClick={(e) => { e.stopPropagation(); onToggleFav(); }}
        className="absolute right-2 top-2 z-10 grid h-7 w-7 place-items-center rounded-full transition-all active:scale-90"
        style={{
