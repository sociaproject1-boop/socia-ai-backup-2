...
import { ArrowLeft, Lock, Star, BarChart3, Wallet, TrendingUp, Award, ChevronRight, Heart } from "lucide-react";
...
const FEATURES = [
  { icon: Star,       title: "Creator Stars",      desc: "Receive stars from fans. Each star converts to creator earnings." },
  { icon: Wallet,     title: "Creator Payouts",     desc: "Withdraw earnings directly to GCash, Maya, or bank." },
  { icon: BarChart3,  title: "Earnings Dashboard",  desc: "Real-time earnings analytics, top content, fan breakdown." },
  { icon: TrendingUp, title: "Creator Levels",      desc: "Bronze → Silver → Gold → Diamond — unlock perks per level." },
  { icon: Award,      title: "Creator Badges",      desc: "Verified, Top Creator, Rising Star — earned through activity." },
  { icon: Heart,      title: "Fan Support",         desc: "Fans can send one-time or monthly support directly to you." },
];

const REQUIREMENTS = ["5,000+ followers", "Active content creator", "Policy compliant account", "Creator approval"];

const ROLLOUT_STEPS = [
  { label: "Funding goal reached", active: true },
  { label: "7–15 days infrastructure prep", active: false },
  { label: "Limited beta — selected creators", active: false },
  { label: "Stability monitoring", active: false },
  { label: "Gradual public rollout", active: false },
];
  ...
  return (
    <div className="app-bg min-h-[100dvh] pb-28">
      <div className="app-header sticky top-0 z-40 flex items-center gap-3 px-4 py-3">
        ...
        <h1 className="text-lg font-bold app-text">Creator Monetization</h1>
      </div>
      ...
      <div className="px-4 pt-2">
        ...
        <div className="mt-5">
          ...
          <div className="grid grid-cols-1 gap-3">
            {FEATURES.map((f, i) => (
              <motion.div key={f.title} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }}
...
function LockedBanner({ pct, phase, onSupport }: { pct: number; phase: number; onSupport: () => void }) {
  return (
    <div className="relative overflow-hidden rounded-[22px] p-5 text-center"
      ...
      <h2 className="mb-2 font-display text-[18px] font-black leading-tight text-white">
        Deployment Phase Pending
      </h2>
      <p className="mb-4 text-[11.5px] leading-relaxed text-white/50" style={{ maxWidth: 270, margin: "0 auto 16px" }}>
        Funding goal completion unlocks the next deployment phase. Features roll out gradually after infrastructure preparation and stability checks.
      </p>
