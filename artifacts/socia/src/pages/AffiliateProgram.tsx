...
import { ArrowLeft, Lock, Users, Link2, BarChart3, Gift, DollarSign, Target, Heart, ChevronRight } from "lucide-react";
...
const FEATURES = [
  { icon: Link2,      title: "Product Linking",       desc: "Share affiliate links to products in your content." },
  { icon: DollarSign, title: "Commission Tracking",   desc: "Real-time earnings from clicks, sales, and referrals." },
  { icon: BarChart3,  title: "Campaign Center",       desc: "Join brand campaigns and sponsored creator programs." },
  { icon: Gift,       title: "Creator Perks",         desc: "Exclusive deals, early access, and creator rewards." },
  { icon: Target,     title: "Performance Analytics", desc: "CTR, conversions, earnings per post — all in one view." },
  { icon: Users,      title: "Creator Network",       desc: "Connect with brands and co-creators in the marketplace." },
];

const REQUIREMENTS = ["500+ followers", "Active creator account", "Policy compliant", "Approved affiliate application"];

const ROLLOUT_STEPS = [
  { label: "Funding goal reached", active: true },
  { label: "7–15 days infrastructure prep", active: false },
  { label: "Affiliate system integration & testing", active: false },
  { label: "Limited beta — approved creators first", active: false },
  { label: "Gradual expansion", active: false },
];
  ...
  return (
    <div className="app-bg min-h-[100dvh] pb-28">
      <div className="app-header sticky top-0 z-40 flex items-center gap-3 px-4 py-3">
        ...
        <h1 className="text-lg font-bold app-text">Affiliate Program</h1>
      </div>
  ...
  return (
    <div className="relative overflow-hidden rounded-[22px] p-5 text-center"
      ...
      <p className="mb-4 text-[11.5px] leading-relaxed text-white/50" style={{ maxWidth: 270, margin: "0 auto 16px" }}>
        Funding goal completion unlocks the next deployment phase. The affiliate system rolls out gradually after infrastructure preparation, integration testing, and stability checks.
      </p>
