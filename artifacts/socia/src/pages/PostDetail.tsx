...
export default function PostDetail() {
  ...
  const handleLike = useCallback(async () => {
    if (!post || likeWorking.current) return;
    likeWorking.current = true;
    const prev = { liked: !!post.has_liked, count: post.like_count };
    const nowLiked = !prev.liked;
    setPost((p) => p ? { ...p, has_liked: nowLiked, like_count: Math.max(0, p.like_count + (nowLiked ? 1 : -1)) } : p);
    try {
      const result = await toggleLike(post.id);
      setPost((p) => {
        if (!p) return p;
        const delta = result.liked === p.has_liked ? 0 : result.liked ? 1 : -1;
        return { ...p, has_liked: result.liked, like_count: Math.max(0, p.like_count + delta) };
      });
    } catch {
      setPost((p) => p ? { ...p, has_liked: prev.liked, like_count: prev.count } : p);
    } finally {
      likeWorking.current = false;
    }
  }, [post]);
